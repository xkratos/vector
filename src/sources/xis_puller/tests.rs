...
use crate::sources::util::xis_puller::vrl::VrlDeserializerOptions;
...
pub(crate) const TIME_RANGE: Duration = Duration::from_secs(300);

pub(crate) fn build_vrl_options_from_str(vrl: &str) -> VrlDeserializerOptions {
...
pub(crate) async fn run_compliance(config: PullerConfig) -> Vec<Event> {
    let events =
...
fn http_client_generate_config() {
    test_generate_config::<PullerConfig>();
}

impl ValidatableComponent for PullerConfig {
...
register_validatable_component!(PullerConfig);
...
async fn bytes_decoding() {
    ...
    run_compliance(PullerConfig {
        endpoint: format!("http://{}/endpoint", in_addr),
        interval: INTERVAL,
        timeout: TIMEOUT,
        url_vrl: build_vrl_options_from_str(". = ."),
        cursor_vrl: build_vrl_options_from_str(". = ."),
...
async fn json_decoding_newline_delimited() {
    ...
    run_compliance(PullerConfig {
        ...
        timeout: TIMEOUT,
        url_vrl: build_vrl_options_from_str(". = ."),
        cursor_vrl: build_vrl_options_from_str(". = parse_json!(string!(.))"),
        time_range: TIME_RANGE,
        query: HashMap::new(),
        decoding: DeserializerConfig::Json(Default::default()),
...
async fn request_query_applied() {
    ...
    let events = run_compliance(PullerConfig {
