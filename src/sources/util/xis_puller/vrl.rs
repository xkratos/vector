...
use vector_lib::codecs::decoding::format::BytesDeserializerConfig;
use vector_lib::config::{DataType, LogNamespace};
use vector_lib::event::{Event, TargetEvents, VrlTarget};
use vector_lib::{compile_vrl, schema};
...
fn parse_bytes(bytes: Bytes) -> Event {
    ...
    let log_event = bytes_deserializer.parse_single(bytes, LogNamespace::Vector);
    Event::from(log_event)
}

impl VrlDeserializer {
    pub fn parse(&self, bytes: Bytes) -> vector_common::Result<SmallVec<[Event; 1]>> {
        let event = parse_bytes(bytes);
        self.parse_event(event)
    }
    fn parse_event(&self, event: Event) -> vector_common::Result<SmallVec<[Event; 1]>> {
        ...
        match runtime.resolve(&mut target, &self.program, &self.timezone) {
            Ok(_) => match target.into_events(LogNamespace::Vector) {
    ...
    pub fn parse_cursor(&self, bytes: Bytes) -> vector_common::Result<serde_json::Value> {
        let cursor = self.parse(bytes)?;
        match cursor.first() {
            Some(event) => {
                let cursor_json = event.as_log().get(&OwnedTargetPath::event_root()).unwrap();
                let cursor_json_str = cursor_json.to_string();
                let v: serde_json::Value = serde_json::from_str(&cursor_json_str).unwrap();
                if v.is_object() {
                    Ok(v)
                } else {
                    Err("Cursor is not a JSON object".into())
                }
            }
            None => Err("No cursor found in event".into()),
        }
    }

    pub fn build_urls(&self, cursor: serde_json::Value) -> vector_common::Result<Vec<String>> {
        match Event::from_json_value(cursor, LogNamespace::Vector) {
            Ok(cursor) => {
                let url_events = self.parse_event(cursor)?;
                Ok(url_events
                    .iter()
                    .map(|url| {
                        url.as_log()
